...
Все имена полей — `snake_case`; все Pydantic v2 модели запрещают неизвестные поля. Обязательные поля должны присутствовать. Запрос `ScenarioRequest` для validate и evaluate содержит только `model_version: str` и `selections: list[SelectionSchema]`. Запрос `ExplainRequest` для explain расширяет его необязательным `language: "ru" | "kk" | "en"`, по умолчанию `"ru"`. Другие значения и явный `null` отвергаются с HTTP 422. Поле языка в validate/evaluate также отвергается как неизвестное. Каждое решение содержит только `measure_id: str` и опциональный `district_id: str`. Если район отсутствует, транспортная модель ставит внутреннее `None`; явный JSON `null` отвергается схемой. Проверку соответствия району/городу делает доменный валидатор по каталогу. Клиент не передаёт бюджет, эффект, Score, prompt, ID модели или отчёт.
...
| `POST /api/v1/explain` | `model_version`, `scenario_key`, `language: ru \| kk \| en`, `mode: llm \| fallback`, `llm_model: str \| null`, `explanation: Explanation`, `warning: Warning \| null` |
...
В успешном ответе explain поле `language` всегда повторяет принятый язык, в том числе при fallback. Для LLM язык передаётся доверенной системной инструкцией отдельно от фактов. Для fallback все разделы объяснения, названия районов и `warning.message` переведены на выбранный язык. Значения `warning.code` остаются стабильными: `AI_NOT_CONFIGURED`, `AI_BUSY`, `AI_TIMEOUT`, `AI_UNAVAILABLE`, `AI_INVALID_RESPONSE`. Язык не меняет расчёт и `scenario_key`: клиент сопоставляет ответ с текущими ключом сценария и языком. Числовые данные и каталог сохраняют прежний контракт; сообщения ошибок правил локализуются интерфейсом по `code`.

Ответ LLM проверяется по `ExplanationSchema`. Символ Unicode U+00B7 заменяется пробелом во всех текстовых разделах; пустой после очистки текст считается невалидным ответом и приводит к fallback. Числа внутри шаблонного объяснения округляются для чтения до двух знаков, числовой отчёт сохраняет полную точность.
...
Набор проверяет исходный Score и оба пресета, независимость результата от порядка мер, запрет раннего окончательного расчёта, строгие схемы, ASGI-маршруты, три языка fallback, передачу языка через сервис и OpenAI-адаптер, очистку запрещённого символа, тайм-аут и освобождение единственного AI-слота. Провайдер подменяется локальным объектом; это не проверка доступности OpenAI и качества настоящего перевода.
