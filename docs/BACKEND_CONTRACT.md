...
`validate_scenario(final=False)` допускает 0–5 мер, но проверяет остальные применимые правила. `final=True` требует ровно пять. Неизвестная мера или район оставляет `spent` и `remaining` равными `None`; `limit` остаётся известен. Для полностью известного набора `remaining = limit - spent`, даже если число отрицательное. `can_evaluate` истинно только для валидных пяти решений. Порядок решений не влияет на результат: ключ сценария и отчёт сортируют их по числовой части ID меры. Пример ключа: `tech2-v1|M5@saryarka|M7@nura|M8@nura|M10@nura|M12@city`.
