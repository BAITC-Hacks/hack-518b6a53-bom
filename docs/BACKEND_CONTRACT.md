...
Приоритет: (1) текущий запрос пользователя и его явные уточнения; (2) `data.md`, Tech2; (3) `ARCHITECTURE_PYTHON_DOCKER.md`; (4) этот контракт, если он не противоречит первым трём. Tech2 задаёт исходные данные, формулу и бизнес-правила. ID Есиля в API — `yesil`; интерфейс преобразует свой ID `esil` на границе API.
...
Все имена полей — `snake_case`; все Pydantic v2 модели запрещают неизвестные поля. Обязательные поля должны присутствовать. Запрос `ScenarioRequest` для validate и evaluate содержит только `model_version: str` и `selections: list[SelectionSchema]`. Запрос `ExplainRequest` для explain расширяет его необязательными `language: "ru" | "kk" | "en"` (по умолчанию `"ru"`) и `district_id: str` верхнего уровня. Для городского объяснения `district_id` опускается, для районного передаётся известный ID из каталога. Неизвестный район возвращает HTTP 422 с `UNKNOWN_DISTRICT` и `path="district_id"` до вызова провайдера. Неизвестный язык и явный `null` в языке или районе отвергаются схемой с HTTP 422. Дополнительные поля языка и области в validate/evaluate запрещены. Каждое решение содержит только `measure_id: str` и опциональный `district_id: str`. Если район решения отсутствует, транспортная модель ставит внутреннее `None`; явный JSON `null` отвергается схемой. Проверку соответствия району/городу делает доменный валидатор по каталогу. Клиент не передаёт бюджет, эффект, Score, prompt, ID модели или отчёт.
...
| `POST /api/v1/explain` | `model_version`, `scenario_key`, `language: ru \| kk \| en`, `district_id: str \| null`, `mode: llm \| fallback`, `llm_model: str \| null`, `explanation: Explanation`, `warning: Warning \| null` |
...
В успешном ответе explain поля `language` и `district_id` всегда повторяют принятую область, в том числе при fallback; для города `district_id=null`. Язык и область не меняют числовой расчёт или `scenario_key`: клиент сопоставляет ответ с тройкой ключ сценария + язык + район. Для LLM язык передаётся доверенной системной инструкцией отдельно от фактов. Fallback содержит один короткий абзац для выбранной области и языка; `warning.message` также переведён. Значения `warning.code` остаются стабильными: `AI_NOT_CONFIGURED`, `AI_BUSY`, `AI_TIMEOUT`, `AI_UNAVAILABLE`, `AI_INVALID_RESPONSE`. Числовые данные и каталог сохраняют прежний контракт; сообщения ошибок правил локализуются интерфейсом по `code`.

Числовой и шаблонный отчёт интерфейс показывает сразу. Explain вызывается только по явной кнопке AI; открытие отчёта, переключение района и языка сами по себе генерацию не запускают.

Провайдеру передаётся компактный контекст из `backend/services/explanation_context.py`. Для города это агрегаты, оценки районов с готовыми дельтами, все ненулевые изменения показателей в `indicator_deltas`, принятые меры и каталог рекомендаций. `measure_effects` передаёт рассчитанный вклад каждой выбранной меры: стоимость, лаг, реализованную долю и добавки по районам после лага, до clip; синергии передаются отдельно. Четыре основных изменения и три слабых показателя остаются краткой сводкой, которая не заменяет полные дельты. Для района передаются его десять показателей и только применимые меры, включая общегородские; вложенные добавки общегородских мер также ограничены выбранным районом. Изменения соседних районов в районный контекст не входят. Готовые вклады и полные дельты сохраняют вычисленную точность; краткие сводки округлены до двух знаков. Числовой HTTP-отчёт не меняется, пересчёт со стороны LLM не требуется.

Модель получает простую схему `BriefExplanation` с единственным полем `summary`. Инструкция требует один абзац примерно 50–75 слов с выводом и рекомендациями, без списков и заголовков, не более трёх числовых значений с максимум двумя знаками после запятой. Пробелы и переносы нормализуются в один абзац; Unicode U+00B7 заменяется пробелом. Пустой после очистки текст вызывает fallback. Для совместимости HTTP-ответ сохраняет `ExplanationSchema`: абзац помещается в `summary`, массивы `strengths`, `risks`, `recommendations` остаются пустыми. Качество и фактическое соблюдение рекомендаций в свободном тексте требуют отдельной проверки реальным провайдером.

При настоящем `http.disconnect` обработчик explain отменяет задачу сервиса, дожидается освобождения единственного AI-слота и завершает обработку с внутренним кодом 499 `CLIENT_DISCONNECTED`. Потреблённое тело или пустое сообщение `http.request` отключением не считаются. Очереди запросов нет.
...
`backend/settings.py` читает только инфраструктуру: `DATA_DIR` (по умолчанию `/app/data/tech2-v1`), `LOG_LEVEL` (`INFO`), `OPENAI_API_KEY` (нет значения по умолчанию), `OPENAI_MODEL` (нет значения по умолчанию), `OPENAI_TIMEOUT_SECONDS` (`45`) и `OPENAI_MAX_OUTPUT_TOKENS` (`900`). Адаптер краткого объяснения ограничивает фактический `max_output_tokens` минимумом настройки и 900, поэтому старое значение 2000 не возвращает длинную генерацию. Для выбранного `gpt-6-luna` передаётся `reasoning={"effort":"none"}`; имя модели и провайдер не меняются. Поддержка этого режима подтверждена [официальной страницей модели](https://developers.openai.com/api/docs/models/gpt-6-luna); назначение лимита описано в [руководстве Responses reasoning](https://developers.openai.com/api/docs/guides/reasoning). Это ограничивает объём работы, но не гарантирует конкретное время ответа.

Пустые ключ и модель означают отсутствие AI-конфигурации и не препятствуют загрузке данных, валидации и расчёту. Бюджет, лаги, веса, пороги, эффекты, конфликты и синергии не задаются переменными окружения. `/health/ready` зависит от датасета и базового расчёта, а не от OpenAI; `/health/ai` сообщает локальную конфигурацию и результат последнего вызова без внешнего запроса.
...
`tests/test_backend.py` использует `unittest` из стандартной библиотеки и рабочие зависимости API. Команда внутри контейнера с подключённым каталогом `tests`: `python -m unittest discover -s tests -p test_backend.py -v`. Внешние сервисы и настоящий ключ для этих проверок не требуются.

Набор проверяет исходный Score и оба пресета, независимость результата от порядка мер, запрет раннего окончательного расчёта, строгие схемы, ASGI-маршруты, три языка fallback, все районные области, отсутствие ложного изменения для незатронутого района, компактность контекста и передачу только применимых мер. Проверяются схема одного абзаца, выбранные параметры генерации, очистка запрещённого символа, тайм-аут, отсутствие ложной отмены по концу тела запроса и освобождение единственного AI-слота при отключении. Провайдер подменяется локальным объектом; это не проверка доступности OpenAI и качества настоящего перевода.
