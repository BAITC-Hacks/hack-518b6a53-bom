...
Провайдеру передаётся компактный контекст из `backend/services/explanation_context.py`. Для города это агрегаты, оценки районов, четыре основных изменения, три слабых показателя, принятые меры и каталог рекомендаций. Для района это его десять показателей, оценка, слабые места, синергии и только применимые меры, включая общегородские. Изменения соседних районов в районный контекст не входят. Все эти факты вычислены сервером; округление до двух знаков выполняется только для представления AI, числовой отчёт сохраняет полную точность.
