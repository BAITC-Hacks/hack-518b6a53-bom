...
- AI-объяснение сильных сторон, рисков и возможных улучшений сценария.
...
Для AI-разбора заполните в `.env.local` **оба** поля: `OPENAI_API_KEY` и `OPENAI_MODEL` — ключ и доступный API ID модели с поддержкой структурированных ответов Responses API. Модель по умолчанию не задана. После изменения настроек повторите команду запуска. Без конфигурации AI или при ошибке OpenAI приложение показывает явно обозначенное шаблонное объяснение; расчёт продолжает работать. Ключ не добавляйте в Git.
...
JavaScript-тестам нужен Node.js ≥ 22; установка npm-пакетов не требуется:

```sh
npm test
```

Часть текущих JavaScript-тестов не инициализирует каталог и падает, в том числе с `Catalog has not been loaded`. Набор требует актуализации.

Python-тесты запускаются с установленными зависимостями и используют подменённые ответы OpenAI — ключ не нужен:
...
Геометрия в [map-geometry.js](map-geometry.js) адаптирована по [Astana districts coloured 2023.png](https://commons.wikimedia.org/wiki/File:Astana_districts_coloured_2023.png), автор **Bogomolov.PL**, лицензия [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/). Уведомление относится только к геометрии карты.
